...
def init_embed(params, options):
    ...
    params['Wemb_B'] = (0.1 * randB).astype(config.floatX)
...
def memLayers(tparams, options, x, xmask, q, qmask, nhops, wmat, use_linear):
    ...
    emb_A_sf = emb_A_m.dimshuffle(0,2,3,1)
    emb_A_w = emb_A_sf * wmat
    emb_A_s = emb_A_w.sum(axis=3) # storyLen * nsamples * dim_word
    ...
    # define memory hops
    def step(a_emb, b_emb, q_vec, xmask_story):
        ...
        q_vec_broad = q_vec.dimshuffle(0,1,'x').repeat(att_q_a_sm.shape[1],axis=2) # nsamples * dim_word * storyLen
        ...
        def in_step(mat):
            return tensor.switch(use_linear, mat, tensor.nnet.softmax(mat))
        ...
        # update a:
        # a_emb_n_pre = tensor.nnet.softplus(a_emb + retrieve_cov_a.dimshuffle(1,0,2) + a_update_with_q.dimshuffle(2,0,1))
        # b_emb_n_pre = tensor.nnet.softplus(b_emb + (b_emb.dimshuffle(2,1,0) * att_q_a_sm).dimshuffle(2,1,0))
        # a_emb_n_pre = tensor.tanh(a_emb + retrieve_cov_a.dimshuffle(1,0,2) + a_update_with_q.dimshuffle(2,0,1))
        # b_emb_n_pre = tensor.tanh(b_emb + (b_emb.dimshuffle(2,1,0) * att_q_a_sm).dimshuffle(2,1,0))
        a_emb_n_pre = retrieve_cov_a.dimshuffle(1,0,2) + a_update_with_q.dimshuffle(2,0,1)
        b_emb_n_pre = b_emb + (b_emb.dimshuffle(2,1,0) * att_q_a_sm).dimshuffle(2,1,0)
        a_emb_n = a_emb_n_pre * xmask_story.dimshuffle(1,0,'x').repeat(a_emb_n_pre.shape[2], axis=2)
        b_emb_n = b_emb_n_pre * xmask_story.dimshuffle(1,0,'x').repeat(b_emb_n_pre.shape[2], axis=2)
        
        
        return a_emb_n, b_emb_n, q_vec_n
        
    states, updates = theano.scan(step, outputs_info=[emb_A_s, emb_B_s, emb_q_s], non_sequences=[xmask_story], n_steps=nhops)
...
def build_model_EvoMN(options, tparams):
    ...
    pred = tensor.nnet.softmax(tensor.dot(proj, tparams['Wclass'].T)) # nsamples * vocab_size
    ...
    try:
        for eidx in xrange(max_epochs):
            ...
            for _, train_index in kf:
                ...
                # [x,xmask,q,qmask,y,nhops,wmat,proj,pred,pred_ans,aEmbSeq,bEmbSeq,qSeq] = f_debug(x, xmask, q, qmask, y, nhops, wordTimeMat)
                # print qSeq[0,1,:]
                # print qSeq[1,1,:]
                # print qSeq[2,1,:]

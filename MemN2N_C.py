...
def init_embed(params, options):
    ...
    params['Wemb_A_1'] = (0.1 * randA).astype(config.floatX)
    for i in range(nhops):
        randB = numpy.random.randn(options['vocab_size'], options['dim_word'])
        name = 'Wemb_B_' + str(i+1)
        params[name] = (0.1 * randB).astype(config.floatX)
    
    # Wclass = numpy.random.rand(options['vocab_size'], options['dim_hidden']) # this is the final matrix to predict answer.
...
def memLayers(tparams, options, x, xmask, q, qmask, nhops, wmat, use_linear):
    ...
    embeddingBList = []
    # q and first hop a 
    emb_A = tparams['Wemb_A_1'][x.flatten()].reshape([x.shape[0], x.shape[1], x.shape[2], options['dim_word']]) # storyLen * sentLen * nsamples * dimword
    emb_A_m = emb_A * xmask.dimshuffle(0,1,2,'x').repeat(emb_A.shape[3], axis=3)    
    emb_A_s = (emb_A_m.dimshuffle(0,2,3,1) * wmat).sum(axis=3)# storyLen * nsamples * dim_word
    embeddingAList.append(emb_A_s)
    
    emb_q = tparams['Wemb_A_1'][q.flatten()].reshape([q.shape[0], q.shape[1], options['dim_word']]) # sentLen * nsamples * dimword
    ...
    for i in range(options['nhops'] - 1):
        name = 'Wemb_B_' + str(i + 1)
        emb_B = tparams[name][x.flatten()].reshape([x.shape[0], x.shape[1], x.shape[2], options['dim_word']])
        emb_B_m = emb_B * xmask.dimshuffle(0,1,2,'x').repeat(emb_B.shape[3], axis=3)
        emb_B_s = (emb_B_m.dimshuffle(0,2,3,1) * wmat).sum(axis=3) # storyLen * nsamples * dim_word
        embeddingBList.append(emb_B_s)
        embeddingAList.append(emb_B_s)
        
    emb_out = tparams['Wemb_B_' + str(options['nhops'])][x.flatten()].reshape([x.shape[0], x.shape[1], x.shape[2], options['dim_word']])
    emb_out_m = emb_out * xmask.dimshuffle(0,1,2,'x').repeat(emb_out.shape[3], axis=3)
    emb_out_s = (emb_out_m.dimshuffle(0,2,3,1) * wmat).sum(axis=3) # storyLen * nsamples * dim_word
    embeddingBList.append(emb_out_s)
    
    A_list = tensor.stacklists(embeddingAList)
    B_list = tensor.stacklists(embeddingBList)
    # xmask_story = xmask.max(axis=1).dimshuffle(1,0) # nsamples * storyLen
    
    # define memory hops
    def step(a_emb, b_emb, q_vec):
        ...
        att_q_a_sm = tensor.switch(use_linear, att_q_a, tensor.nnet.softmax(att_q_a)) # nsamples * storyLen
        retrieve_b_with_a = ((b_emb.dimshuffle(2,1,0) * att_q_a_sm).sum(axis=2)).dimshuffle(1,0) # nsamples * dim_word
        # retrieve_a_with_a = ((a_emb.dimshuffle(2,1,0) * att_q_a_sm).sum(axis=2)).dimshuffle(1,0) # nsamples * dim_word
        ...
        q_vec_n = q_vec + retrieve_b_with_a # nsamples * dim_word
        
        '''# broadcast q with attention
        q_vec_broad = q_vec.dimshuffle(0,1,'x').repeat(att_q_a.shape[1],axis=2) # nsamples * dim_word * storyLen
        att_q_a_spam = att_q_a_sm.dimshuffle(0,'x',1).repeat(q_vec.shape[1],axis=1) # nsamples * dim_word * storyLen
        a_update_with_q = q_vec_broad * att_q_a_spam # nsamples * dim_word * storyLen
        ...
        # update a:
        a_emb_n = tensor.nnet.softplus(a_emb + retrieve_cov_a.dimshuffle(1,0,2) + a_update_with_q.dimshuffle(2,0,1))
        b_emb_n = tensor.nnet.softplus(b_emb + (b_emb.dimshuffle(2,1,0) * att_q_a_sm).dimshuffle(2,1,0))'''
        
        
        return q_vec_n
        
    states, updates = theano.scan(step, sequences=[A_list, B_list], 
                                        outputs_info=[emb_q_s], n_steps=nhops)
    return A_list, B_list, states
...
def sgd(lr, tparams, grads, x, xmask, q, qmask, y, nhops, wmat, cost):
    ...
    # updates the weights.
    f_grad_shared = theano.function([x, xmask, q, qmask, y, nhops, wmat], cost, updates=gsup,
...
def debug_print(f_debug, f_grad, prepareData, x_data, q_data, y_data, nhops, wmat, maxStoryLen, maxSentLen, tparams, sliced_iterator):
    ...
    x, xmask, q, qmask, y = prepareData([x_data[t] for t in valid_index],
                                  ...
                                  [y_data[t] for t in valid_index],
                                  maxStoryLen, maxSentLen)
    [x,xmask,q,qmask,y,nhops,wmat,proj,pred,pred_ans,aEmbSeq,bEmbSeq,qSeq] = f_debug(x, xmask, q, qmask, y, nhops, wmat)
...
def train_EvoMN(
    dim_word=50, # dim for word embedding, which is equivalent to GRU hidden layer of evolution.
    dim_hidden=50, # dim for reasoning, which is equivalent to the system status.
    vocab_size=200,
    nhops=3,
    mem_size=50,
    sent_size=100,
    ...
    use_noise, use_linear, x, xmask, q, qmask, y, nhops_v, wmat, proj, pred, pred_ans, cost, f_debug, f_pred, f_ans = build_model_EvoMN(model_options, tparams)
    # do L2 nomalization
    if decay_c > 0.:
        if decay_c > 0.:
    ...
    kf_valid = get_minibatches_idx(len(q_valid), valid_batch_size, shuffle=True)  # kf_valid: a list of tuples. [(id, batch_index)]
    kf_test = get_minibatches_idx(len(q_test), valid_batch_size, shuffle=True)  # these two include all batch indices.
    ...
    use_linear.set_value(1.)

    try:
        for eidx in xrange(max_epochs):
            if eidx >= linear_epochs:
                use_linear.set_value(0.)
            n_samples = 0
    ...
    valid_err = pred_error(f_ans, prepareData, story_valid, q_valid, ans_valid, nhops, wordTimeMat, maxStoryLen, maxSentLen, 1, kf_valid)
    test_err = pred_error(f_ans, prepareData, story_test, q_test, ans_test, nhops, wordTimeMat, maxStoryLen, maxSentLen, 1, kf_test)
